module.exports = async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: 'OPENAI_API_KEY is missing in Vercel.' });
  ...
  if (imageData.length > 6000000) {
    return res.status(413).json({ error: 'Please use an image under 4 MB.' });
  }
...
Rules: extract only facts visible in the image; never guess; use empty strings or arrays when absent; keep list items short; if no reminder date is visible, suggest bringing it back one week before Teacher Appreciation Week; return JSON only.`;
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ error: 'Could not process the image. Check Vercel function logs.' });
  }
};
